...
// Trabalhos NOK (não-OK), fora do dashboard. Permite recuperar (voltar a PENDENTE).
export default function NokList() {
  ...
  async function reload() {
    setLoading(true);
    try { const d = await api.listWorks({ estado: 'NOK' }); setItems(d.works); setError(''); }
    catch (e) { setError(e.message); } finally { setLoading(false); }
  }
  useEffect(() => { reload(); }, []);

  async function recuperar(w) {
    if (!confirm(`Recuperar "${w.pm || w.id_ordem}"? Volta ao estado Pendente e reaparece no dashboard.`)) return;
    setBusy(w.id);
    try {
      await api.updateWork(w.id, { estado: 'PENDENTE' });
      setItems((xs) => xs.filter((x) => x.id !== w.id));
      window.dispatchEvent(new Event('fc-nok-changed'));
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 space-y-4">
      <div className="flex items-center gap-2">
        <h1 className="text-lg font-bold text-slate-800">NOK</h1>
        <span className="rounded-full bg-red-100 text-red-700 text-sm px-2 py-0.5">{items.length}</span>
        <button onClick={reload} className="ml-auto text-sm text-brand underline">atualizar</button>
      </div>
      <p className="text-xs text-slate-500">Trabalhos marcados NOK — fora do dashboard. Recupera quando estiverem prontos para voltar ao fluxo.</p>
      {error && <p className="text-sm text-red-600">{error}</p>}

      {loading ? (
        <p className="text-sm text-slate-500">A carregar…</p>
      ) : items.length === 0 ? (
        <p className="rounded-xl border border-slate-200 bg-white p-6 text-center text-sm text-slate-500">Sem trabalhos NOK. 🎉</p>
      ) : items.map((w) => (
